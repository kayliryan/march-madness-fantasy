...
- ~~2026 dataset incomplete~~ — **resolved 2026-07-18**: the missing 13 games (all 4 First Four + 9 r64) were discovered via ESPN scoreboard sweep and merged; the dataset now has all 68 teams / 718 players / 67 games with completeness assertions. ESPN's API is directly reachable from this environment (`site.api.espn.com` — no proxy workarounds needed), which matters for building the live 2027 provider.
